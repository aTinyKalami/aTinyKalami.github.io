...
汇总表格可以看到，主要参数都集中在MoE部分（FFN），即MoE专家，占657B。尤其是其中的256个路由专家，参数量占到了651B。但推理时只激活其中一小部分专家（8个路由+1个共享），所以MoE部分的activated Parameter会少很多（大概24B），比稠密模型的FFN在运行时的参数量负载低很多。
